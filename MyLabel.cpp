...
//        // force everything fully opaque (no transparency keying)
//        for (int y = 0; y < a.height(); y++) {
//            QRgb *line = reinterpret_cast<QRgb*>(a.scanLine(y));
//            for (int x = 0; x < a.width(); x++) {
//                QRgb c = line[x];
...
//        }
        ...
        QImage a = m_lastImage.convertToFormat(QImage::Format_RGB888);
    ...
    // 3) custom web overlay images
    QString imagesDir = QDir(QCoreApplication::applicationDirPath()).filePath("webapp/images");
    for (const auto& ov : m_cfg.overlays) {
        if (!ov.enabled || ov.file.isEmpty()) continue;

        QString full = QDir(imagesDir).filePath(ov.file);
        if (!m_overlayCache.contains(full)) {
            m_overlayCache.insert(full, QImage(full));
        }

        QImage img = m_overlayCache.value(full);
        if (img.isNull()) continue;

        int ow = ov.w > 0 ? ov.w : img.width();
        int oh = ov.h > 0 ? ov.h : img.height();
        if (ow <= 0 || oh <= 0) continue;

        p.save();
        p.setOpacity(ov.opacity);
        p.drawImage(QRectF(ov.x, ov.y, ow, oh), img);
        p.restore();
    }
